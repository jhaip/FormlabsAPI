/**
 * PreForm API
 * No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)
 *
 * The version of the OpenAPI document: 1.0
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 *
 */

import ApiClient from '../ApiClient';
import ModelsSelectionModelModels from './ModelsSelectionModelModels';

/**
 * The ModelsSelectionModel model module.
 * @module model/ModelsSelectionModel
 * @version 1.0
 */
class ModelsSelectionModel {
    /**
     * Constructs a new <code>ModelsSelectionModel</code>.
     * @alias module:model/ModelsSelectionModel
     */
    constructor() { 
        
        ModelsSelectionModel.initialize(this);
    }

    /**
     * Initializes the fields of this object.
     * This method is used by the constructors of any subclasses, in order to implement multiple inheritance (mix-ins).
     * Only for internal use.
     */
    static initialize(obj) { 
    }

    /**
     * Constructs a <code>ModelsSelectionModel</code> from a plain JavaScript object, optionally creating a new instance.
     * Copies all relevant properties from <code>data</code> to <code>obj</code> if supplied or a new instance if not.
     * @param {Object} data The plain JavaScript object bearing properties of interest.
     * @param {module:model/ModelsSelectionModel} obj Optional instance to populate.
     * @return {module:model/ModelsSelectionModel} The populated <code>ModelsSelectionModel</code> instance.
     */
    static constructFromObject(data, obj) {
        if (data) {
            obj = obj || new ModelsSelectionModel();

            if (data.hasOwnProperty('models')) {
                obj['models'] = ModelsSelectionModelModels.constructFromObject(data['models']);
            }
        }
        return obj;
    }

    /**
     * Validates the JSON data with respect to <code>ModelsSelectionModel</code>.
     * @param {Object} data The plain JavaScript object bearing properties of interest.
     * @return {boolean} to indicate whether the JSON data is valid with respect to <code>ModelsSelectionModel</code>.
     */
    static validateJSON(data) {
        // validate the optional field `models`
        if (data['models']) { // data not null
          ModelsSelectionModelModels.validateJSON(data['models']);
        }

        return true;
    }


}



/**
 * @member {module:model/ModelsSelectionModelModels} models
 */
ModelsSelectionModel.prototype['models'] = undefined;






export default ModelsSelectionModel;

