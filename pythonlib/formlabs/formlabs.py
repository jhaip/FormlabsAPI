...
def _find_preform_server(pathToPreformServer=None):
    ...
    preformserver_location = os.path.join(library_path, filename)
    if not os.path.isfile(preformserver_location):
        raise FileNotFoundError("PreFormServer executable not found at " + preformserver_location)

    return preformserver_location
